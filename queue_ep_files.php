...
verbose(array("outputMode" => $verbose_output_mode, "outputMessage" => "Getting the directory listing from path '".$ep_file_path.$epfile_folder."'", "logName" => "queue_ep_files_php"));
...
for ($x = 0; $x < sizeof($main_directory_listing); $x++) {
    if (in_array($main_directory_listing[$x], $ignored_files)) {
        verbose(array("outputMode" => $verbose_output_mode, "outputMessage" => "'".$main_directory_listing[$x]."' is not an accepted file or directory, skipping", "logName" => "queue_ep_files_php"));
        continue;
    }
    $folder_nydate = $main_directory_listing[$x];

    verbose(array("outputMode" => $verbose_output_mode, "outputMessage" => "Checking if folder ".$folder_nydate." is empty...", "logName" => "queue_ep_files_php"));
    ...
    if ($is_folder_empty) {
        verbose(array("outputMode" => $verbose_output_mode, "outputMessage" => "Directory '$folder_nydate' is empty, skipping", "logName" => "queue_ep_files_php"));
        continue;
    } else {
        verbose(array("outputMode" => $verbose_output_mode, "outputMessage" => "Directory '$folder_nydate' is not empty", "logName" => "queue_ep_files_php"));
        ...
        for ($z = 0; $z < sizeof($temp_file_listing); $z++) {
            ...
            if (in_array($temp_file, $ignored_files)) {
                verbose(array("outputMode" => $verbose_output_mode, "outputMessage" => "$temp_file' is not an accepted file or directory, skipping", "logName" => "queue_ep_files_php"));
                continue;
            }
            verbose(array("outputMode" => $verbose_output_mode, "outputMessage" => "Checking whether file '$temp_file' is already queued...", "logName" => "queue_ep_files_php"));
            ...
            if ($queryData[1] == 0) {
                verbose(array("outputMode" => $verbose_output_mode, "outputMessage" => "The file has not been queued, queuing", "logName" => "queue_ep_files_php"));
                $query_args = array(
                    'filename' => $temp_file,
                ...
                verbose(array("outputMode" => $verbose_output_mode, "outputMessage" => "The file has already been queued, skipping", "logName" => "queue_ep_files_php"));
                continue;
            }
        }
    }
